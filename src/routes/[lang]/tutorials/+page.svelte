    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="space-y-6">
          {#each data.articles as article}
            <article class="border rounded-lg p-6 hover:shadow-md transition-shadow">
              <a href="/{data.lang}/tutorials/{article.slug}">
      ...
      {:else}
        <p class="text-gray-500">
          {data.lang === 'es' ? 'No hay tutoriales disponibles aún.' : data.lang === 'en' ? 'No tutorials available yet.' : 'Nenhum tutorial disponível ainda.'}
        </p>
      {/if}
    </div>
  </div>
