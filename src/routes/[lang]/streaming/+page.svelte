    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="space-y-6">
          {#each data.articles as article}
            <article class="border rounded-lg p-6 hover:shadow-md transition-shadow">
              <a href="/{data.lang}/streaming/{article.slug}">
      ...
      {:else}
        <p class="text-gray-500">
          {data.lang === 'es' ? 'No hay contenido de streaming disponible aún.' : data.lang === 'en' ? 'No streaming content available yet.' : 'Nenhum conteúdo de streaming disponível ainda.'}
        </p>
      {/if}
    </div>
  </div>
